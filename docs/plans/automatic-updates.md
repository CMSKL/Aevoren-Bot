...
- 新版本自动下载并展示整数百分比进度。
- 下载完成后写入 `update.pendingVersion` 收据；新版本成功启动且版本不低于收据时，显示“已更新”并清除收据。
...
Beta/Stable 都使用构建时固化的无凭据 HTTPS 通用更新源。远程源码仓库为 Private，因此不允许把 GitHub PAT 嵌入客户端读取私有 Release。

## 可信发布链

`build/electron-builder.config.cjs` 生成 macOS `dmg + zip`、blockmap 和频道元数据。ZIP 是 Squirrel.Mac 更新所需目标，元数据包含包大小与 SHA-512。Release 构建只接受没有用户名、密码、查询参数或 Fragment 的 HTTPS Base URL。
...
6. 先上传不可变的 ZIP/DMG/blockmap，最后上传 `beta-mac.yml` 或 `latest-mac.yml`；
7. 频道元数据禁用缓存，版本化产物使用一年 immutable 缓存。

CI 所需 Secrets：

- `CSC_LINK`、`CSC_KEY_PASSWORD`
- `APPLE_ID`、`APPLE_APP_SPECIFIC_PASSWORD`、`APPLE_TEAM_ID`
- `AEVOREN_UPDATE_BASE_URL`
- `AEVOREN_R2_ENDPOINT`、`AEVOREN_R2_BUCKET`
- `AEVOREN_R2_ACCESS_KEY_ID`、`AEVOREN_R2_SECRET_ACCESS_KEY`

任一 Secret 缺失时发布任务 Fail Closed，不会产出或覆盖频道元数据。
...
```bash
AEVOREN_RELEASE_BUILD=1 \
AEVOREN_UPDATE_BASE_URL=https://your-public-update-host.example/aevoren-bot \
pnpm release:mac
```

Release 构建还必须提供签名和公证环境变量。本地 Development 不需要、也不应伪装成已公证发布包。
