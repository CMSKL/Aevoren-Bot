...
- 一次性 Approval 与持久化 Tool Journal 基础合同，以及由用户显式授权的 Workspace Registry；当前没有文件正文读取或命令执行入口。
...
- 数据库当前为 v10：v3～v7 增加 Room、多 Agent Runtime/Handoff、自动路由、拒绝审计和 Bot 侧边栏状态，v8 增加显式 Memory，v9 增加 Approval 与 Tool Journal，v10 增加 Workspace Registry；全部迁移按版本顺序执行并保留旧逻辑记录；
